...
**问题描述**：在一次打字中断联问题的排查中，成功抓取到了如下异常日志：
`[00:09:18.756,652] <dbg> zmk: le_param_updated: F4:4E:FC:9F:94:E6 (public): interval 12 latency 0 timeout 400`
随即触发了 `0x22` 错误断开连接。

**日志深度解析**：
*   **interval 12**：键盘设定每 15 毫秒就要和电脑通一次信，属于极其高频低延迟的设定。
*   **latency 0**：要求零容错，键盘不可跳过任何一次回应。
*   **timeout 400**：死亡倒计时仅为 4 秒。只要由于各种原因（如系统卡顿、2.4GHz 干扰）导致连续 4 秒没收到心跳包，即彻底判定断开（抛出 `0x22 Timeout`）。
