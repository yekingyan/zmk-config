...
├─────┼─────┼─────┼─────┼─────┼─────┼─────┐   ┌─────┼─────┼─────┼─────┼─────┼─────┼─────┤
│     │C(Z) │C(X) │C(C) │C(V) │     │(none)│   │(none)│C(BS)│C(DEL)│ C-U │ C-D │PgDn │     │
└─────┴─────┴─────┼─────┼─────┼─────┼─────┤   ├─────┼─────┼─────┼─────┴─────┴─────┴─────┘
                  │     │▓▓▓▓▓│     │(none)│   │(none)│     │     │     │
                  └─────┴─────┴─────┴─────┘   └─────┴─────┴─────┴─────┘
```
...
- 左手布局重组：
  - **BSPC (T键)**：拇指区已有一个 BSPC，此处作备用放食指拉伸位。
- 右手收纳于 3x5 核心区：
  - **垂直功能通道**：小指列从上到下为 `DEL` → `PgUp` → `PgDn`。
  - **跳跃线**：`C(←)` / `C(→)` 环绕 `END` / `HOME`。
- 其他：左手下行保留剪贴板操作，右手下行保留跳删及 Vim 翻页。
...
- 左手 = 计算器式"数字 + 算术运算符"：九宫格数字 + 右侧 `-+=`、底行 `.`（小数/IP/版本号）
- 右手 = 编程标点区：三行括号成对排列 `() {} []`，补齐 `! : " ?` 高频符号
- 列助记：col1 `! ~ `` `（`` ` ~ ``同键对）、col4 `_ | \`（三线）、col5 `: " ?`（对话标点，`?` 与 Base `/` 同位）
...
- 左手 = F 键区：与 Sym 层九宫格严格位置对齐（F7=7 位，F4=4 位，F1=1 位）
- 右手 Row 1 = 蓝牙 + 系统：`BT0` / `BT1` / `BT2`，`BTCLR`（清除）和 `BOOT`（刷机）
- 右手 Row 2 = 音量 + 亮度，Row 3 = 媒体控制
