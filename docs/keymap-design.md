...
│     │C(Z) │C(X) │C(C) │C(V) │C(S) │               │     │     │     │     │     │     │
├─────┼─────┼─────┼─────┼─────┼─────┤               ├─────┼─────┼─────┼─────┼─────┼─────┤
│     │CAPS │ ML  │ MU  │ MD  │ MR  │               │     │SHIFT│CTRL │ ALT │ GUI │     │
├─────┼─────┼─────┼─────┼─────┼─────┼─────┐   ┌─────┼─────┼─────┼─────┼─────┼─────┼─────┤
│     │ INS │WH_L │WH_U │WH_D │WH_R │(none)│   │(none)│     │     │     │     │     │     │
└─────┴─────┴─────┼─────┼─────┼─────┼─────┤   ├─────┼─────┼─────┼─────┴─────┴─────┴─────┘
                  │MCLK │RCLK │LCLK │(none)│   │(none)│     │▓▓▓▓▓│     │
                  └─────┴─────┴─────┴─────┘   └─────┴─────┴─────┴─────┘
```

- 左手 = 鼠标移动 + 滚轮 + 剪贴板 + 鼠标按键（右拇指激活，左手主导操作）：
  - 上行 `C(Z) | C(X) | C(C) | C(V) | C(S)`（剪贴板快捷键，与 Nav 层顺序一致）
  - 中行 `CAPS | ML | MU | MD | MR`（Caps Lock + 鼠标移动）
  - 下行 `INS | WH_L | WH_U | WH_D | WH_R`（Insert + 四方向滚轮）
...
### D + F = Ctrl+Space（切换输入法 Combo）

同时按下 `D` 和 `F` 触发 `Ctrl+Space`，用于切换输入法。key-positions `<27 28>`（Lily58 矩阵）。
